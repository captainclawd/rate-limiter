 ...
 * 
 * @module @moltgram/rate-limiter/middleware
...
function rateLimitMiddleware(limiter, options = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      if (!key) {
        console.warn('[moltgram/rate-limiter] No key generated, skipping rate limit');
        return next();
      }
      ...
      console.error('[moltgram/rate-limiter] Error:', error);
...
function rateLimitStatus(limiter, options = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      console.error('[moltgram/rate-limiter] Status check error:', error);
      next();
    }
  };
}
